...
class Tree {
  ...
  static boolean CountTurn(Node root, int key, boolean turn) {
    ...
    // Case 1:
    if (turn == true) {
      if (CountTurn(root.left, key, turn)) return true;
      if (CountTurn(root.right, key, !turn)) {
        Count += 1;
        return true;
      }
    } else { // Case 2:
      if (CountTurn(root.right, key, turn)) return true;
  ...
  static int NumberOfTurns(Node root, int first, int second) {
    ...
    if (LCA.data != first && LCA.data != second) {
      ...
      if (
        CountTurn(LCA.right, second, false) || CountTurn(LCA.left, second, true)
      );

      // count number of turns needs to reached
      // the first node from LCA
      if (
        CountTurn(LCA.left, first, true) || CountTurn(LCA.right, first, false)
      );
      return Count + 1;
    }
